...
mod tests {
    ...
    fn mount(config_path: &str, project_dir: Option<&str>) -> (Context, Arc<ToolRuntime>) {
        let ctx = Context::new();
        ...
        let mut config = json!({ "configPath": config_path });
        if let Some(dir) = project_dir {
            config["projectDir"] = json!(dir);
        }
        install(&ctx, Some(&config)).unwrap();
        (ctx, tools)
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn matching_pre_tool_use_exit_2_denies_and_the_tool_never_runs() {
        let dir = temp_dir();
        ...
        let (ctx, tools) = mount(&config, None);
        let ran = Arc::new(AtomicBool::new(false));
        let flag = Arc::clone(&ran);
        tools.insert(Arc::new(ScriptTool::new("danger", "d", move |_| {
            flag.store(true, Ordering::SeqCst);
            dsh_tools::ToolOutcome::text("no")
        })));
        let denied = tools
            .execute_for(&ctx, "danger", json!({}), None)
            .await
            .unwrap();
        assert!(denied.outcome.is_error);
        assert!(denied.outcome.content.iter().any(|block| match block {
            ContentBlock::Text { text } => text.contains("danger tool blocked"),
            _ => false,
        }));
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn unmatched_pre_tool_use_leaves_the_tool_alone() {
        let dir = temp_dir();
        ...
        let (ctx, tools) = mount(&config, None);
        tools.insert(Arc::new(ScriptTool::new("safe", "s", |_| {
            dsh_tools::ToolOutcome::text("ok")
        })));
        let outcome = tools.execute(&ctx, "safe", json!({})).await.unwrap();
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn pre_tool_use_ask_fails_closed_without_approval() {
        let dir = temp_dir();
        let ask = write_script(
            &dir,
            "ask.sh",
        ...
        let (ctx, tools) = mount(&config, None);
        tools.insert(Arc::new(ScriptTool::new("danger", "d", |_| {
            dsh_tools::ToolOutcome::text("no")
        })));
        let denied = tools
            .execute_for(&ctx, "danger", json!({}), None)
        ...
        assert!(denied.outcome.is_error);
        assert!(denied.outcome.content.iter().any(|block| match block {
            ContentBlock::Text { text } => text.contains("needs approval"),
            _ => false,
        }));
        let _ = std::fs::remove_dir_all(dir);
    }
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn pre_tool_use_stdin_has_a_trailing_newline_and_exports_project_dir() {
        let dir = temp_dir();
        ...
        let (ctx, tools) = mount(&config, Some(&project.display().to_string()));
        tools.insert(Arc::new(ScriptTool::new("echo", "e", |_| {
            dsh_tools::ToolOutcome::text("ok")
        })));
        let _ = tools
            .execute(&ctx, "echo", json!({ "x": 1 }))
            .await
            .unwrap();
        let stdin = std::fs::read_to_string(&stdin_path).unwrap();
        assert!(
            stdin.ends_with('\n'),
            "Claude Code stdin is framed with a trailing newline"
        );
