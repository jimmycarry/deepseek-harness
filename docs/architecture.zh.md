...
`agent/pre-step` 决定模型看到什么。监听器可以改写已领取的消息，也可以直接拒绝它们；首次领取被拒绝或被改写为空时，仍会关闭一个不含步骤的持久轮次，因此日志会记录这次尝试。每个步骤读取插件注册的提示词片段和工具 schema，并针对当前会话渲染运行时上下文贡献。失败的模型请求把适配器路由的 `retryPolicy` 放在 `agent/request-error` 上。
