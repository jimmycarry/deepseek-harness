...
| P1 | 这些 profile 上的耐久性与运维 | projection cache；session-query FTS；启用 `web-fetch-http`；OTel flush 提示；外部子代理 provider；`tool-ask-user`。skill 文件系统监视已在轮询适配器上关闭（[Agent Note](../../implemented/feature/2026-09-13-rust-skill-filesystem-watch.zh.md)）。session-query 精确列表与谱系已关闭（[Agent Note](../../implemented/feature/2026-09-14-rust-session-query-lineage.zh.md)）。session-query 表面追踪与过滤已关闭（[Agent Note](../../implemented/feature/2026-09-14-rust-session-query-surface.zh.md)）。skill 不完整 last-good 已关闭（[Agent Note](../../implemented/feature/2026-09-14-rust-skill-incomplete-snapshot.zh.md)）。SDK `Session.run` 与后代通知合并已关闭（[Agent Note](../../implemented/feature/2026-09-14-rust-sdk-session-run.zh.md)） |
...
ACP 图像仅在挂了附件与 vision 默认模型时广告为 `true`。DeepSeek Files API 上传已在已交付的 vision 路径上关闭；剩余的请求版本工作见 [Files 对齐 Agent Note](../../implemented/feature/2026-09-12-rust-deepseek-files-api.zh.md)。skill 文件系统监视已在 [监视对齐 Agent Note](../../implemented/feature/2026-09-13-rust-skill-filesystem-watch.zh.md) 的轮询适配器上关闭。session-query 精确列表与谱系已在 [谱系对齐 Agent Note](../../implemented/feature/2026-09-14-rust-session-query-lineage.zh.md) 关闭。session-query 表面追踪与过滤已在 [表面对齐 Agent Note](../../implemented/feature/2026-09-14-rust-session-query-surface.zh.md) 关闭。skill 不完整 last-good 已在 [不完整快照 Agent Note](../../implemented/feature/2026-09-14-rust-skill-incomplete-snapshot.zh.md) 关闭。SDK `Session.run` 与后代通知合并已在 [Session.run 对齐 Agent Note](../../implemented/feature/2026-09-14-rust-sdk-session-run.zh.md) 关闭。清单一并记录已关闭的 P0、Files 上传、skill 监视、session-query 谱系、session-query 表面、skill 不完整与 SDK Session.run 行，避免后续改动把它们重新打开。
