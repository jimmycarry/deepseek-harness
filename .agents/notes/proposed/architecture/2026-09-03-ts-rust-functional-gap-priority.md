...
| P1 | Durability and ops on those profiles | projection cache; session-query FTS; enable `web-fetch-http`; OTel flush hint; SDK client helpers; external subagent providers; `tool-ask-user`. Skill filesystem watch is closed on the poll adapter ([Agent Note](../../implemented/feature/2026-09-13-rust-skill-filesystem-watch.md)) |
