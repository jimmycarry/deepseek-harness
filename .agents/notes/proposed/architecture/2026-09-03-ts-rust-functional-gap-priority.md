...
The Rust tree already ships headless, ACP, and JSON-RPC profiles under the 1:1 rule in [the port Agent Note](../../implemented/architecture/2026-08-22-rust-harness-port.md). TypeScript still has 227 packages; Rust has 117 crates. Without an agreed order, the next crate can chase Web UI, packed SQLite schema 17, or a loop edit and still look like progress while remaining P0 is the reported finish-chunk gap and remaining thinner paths (DeepSeek Files API, session-query FTS, skill watch) stay open.
...
| P3 | Opt-in capabilities | `schedule` closed ([Rust MCP / schedule / hooks port](../../implemented/feature/2026-09-04-rust-mcp-schedule-hooks.md)). Remaining: extra context; persistent shell; extra title / query / feedback packages; `skill-badge` |
| P4 | Experimental / cloud / hooks | Claude Code / Codex hooks and `mcp-client` closed ([Rust MCP / schedule / hooks port](../../implemented/feature/2026-09-04-rust-mcp-schedule-hooks.md)). Remaining: e2b; agent-team; dynamic Cordis |
...
- **Treat every TypeScript-only package as equal missing work** — 118 leaves include test harnesses, UI plugins, and build-time generators. Equal priority hides the Linux product-user gaps.
