...
The Rust tree already ships headless, ACP, and JSON-RPC profiles under the 1:1 rule in [the port Agent Note](../../implemented/architecture/2026-08-22-rust-harness-port.md). TypeScript still has 227 packages; Rust has 112 crates. Without an agreed order, the next crate can chase Web UI, packed SQLite schema 17, or a loop edit and still look like progress while remaining P0 is the reported finish-chunk gap and remaining thinner paths (session-query FTS) stay open.
...
| P1 | Durability and ops on those profiles | projection cache; session-query FTS; enable `web-fetch-http`; OTel flush hint; external subagent providers; `tool-ask-user`. Skill filesystem watch is closed on the poll adapter ([Agent Note](../../implemented/feature/2026-09-13-rust-skill-filesystem-watch.md)). Session-query exact list and lineage are closed ([Agent Note](../../implemented/feature/2026-09-14-rust-session-query-lineage.md)). Session-query surface traces and filters are closed ([Agent Note](../../implemented/feature/2026-09-14-rust-session-query-surface.md)). Skill incomplete last-good is closed ([Agent Note](../../implemented/feature/2026-09-14-rust-skill-incomplete-snapshot.md)). SDK `Session.run` and descendant notification merge are closed ([Agent Note](../../implemented/feature/2026-09-14-rust-sdk-session-run.md)) |
...
ACP images advertise `true` only when attachments and a vision default model are mounted. DeepSeek Files API upload is closed on the shipped vision path; remaining request-version work stays on [the Files alignment Agent Note](../../implemented/feature/2026-09-12-rust-deepseek-files-api.md). Skill filesystem watch is closed on the poll adapter in [the watch alignment Agent Note](../../implemented/feature/2026-09-13-rust-skill-filesystem-watch.md). Session-query exact list and lineage are closed in [the lineage alignment Agent Note](../../implemented/feature/2026-09-14-rust-session-query-lineage.md). Session-query surface traces and filters are closed in [the surface alignment Agent Note](../../implemented/feature/2026-09-14-rust-session-query-surface.md). Skill incomplete last-good is closed in [the incomplete-snapshot Agent Note](../../implemented/feature/2026-09-14-rust-skill-incomplete-snapshot.md). SDK `Session.run` and descendant notification merge are closed in [the Session.run alignment Agent Note](../../implemented/feature/2026-09-14-rust-sdk-session-run.md). The inventory records closed P0, Files-upload, skill-watch, session-query lineage, session-query surface, skill incomplete, and SDK Session.run rows so a later change does not reopen them.
